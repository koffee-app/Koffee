     ...
     (SELECT array_to_json(array_agg(row_to_json(A)))
      FROM
        (SELECT profiles.name,
           ...
           (SELECT array_to_json(array_agg(row_to_json(images)))
            FROM images
            WHERE images.id=profiles.userid
              AND images.type='profile_image' ) images
         FROM profiles
         WHERE profiles.username=ANY(albums.artists) ) A) AS artists,

     (SELECT array_to_json(array_agg(row_to_json(images)))
      FROM images
      WHERE images.id=albums.id
        AND (images.type='cover_image'
             OR images.type='header_image') ) AS images
   FROM albums
   WHERE albums.id=$1
     AND albums.published=$2) t
`
